...
if it is assumed/known to be present in the data. (Out of scope.) The priors
chosen need to be be somewhat informative as there is not much information
about the parameters from the data alone. Here, the prior for $\rho$ especially
favors values between 0.1 and 0.2; which means the function outputs will be
highly correlated only when the inputs are very similar.
