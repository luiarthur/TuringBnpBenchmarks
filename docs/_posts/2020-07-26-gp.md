...
sorttable: on
date_last_modified: "30 July, 2020."
---
...
For simplicity (and for certain nice theoretical reasons), in many cases, a
particular constant mean functions ($m(\mathbf{x})$ equals a constant) and
covariance functions that depend only on the distance $d$ between observations
$\mathbf{x}$ and $\mathbf{x'}$ are used. When this is the case, another way to
express a GP model is 
...
$$
\begin{eqnarray}
\mathbf{y} \mid \alpha, \rho &\sim& \text{MvNormal}(\mathbf{0}_N, K_{\alpha, \rho}) \\
\alpha &\sim& \text{LogNormal}(0, 1) \\
\rho &\sim& \text{LogNormal}(-2, 0.1) \\
\end{eqnarray}
$$
...
For simply, the data doesn't have any noise. But noise can easily be modeled,
if it is assumed/known to be present in the data. (Out of scope.)
...
TODO: The posterior inference is similar across the various PPLs.
