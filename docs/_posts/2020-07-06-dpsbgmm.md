...
with $\alpha=3$.
<img src="{{ "/assets/img/dp-sb-gmm/sb.png" | prepend: site.baseurl }}" alt="stickbreak image"/>
...
infinite array cannot be created on a computer...) The finite stick-breaking
model simply places an upper bound on the number of mixture components, which,
if chosen reasonably, allows us to reap the benefits of the DP (a model which
...
$K-1$ elements to be specified. The remaining element must be such that the
...
- ADVI:
    ...
    - Full ADVI was done (i.e. no subsampling of the data as in stochastic
      ADVI)
    - NOTE: ADVI is extremely sensitive to initial values. The best run from
      multiple runs with varying initial values was used.
    - In STAN, the default for is to use 100 samples for ELBO approximation
    ...
    - NOTE: There are discrepancies in the optimizers used in each PPL.
      ...
      used.
    - NOTE: Due to insufficient documentation in Numpyro and TFP, ADVI could
      not be used in those PPLs. Though, the documentation indicates that it is
      possible to use ADVI.
- HMC
    - Step-size: 0.01
...
compared to STAN and Turing (several random initial values were used).  Numpyro
was fastest at HMC and NUTS. STAN was fastest at ADVI. Compile times were
shortest for TFP and Pyro.

I would like to point out (though subjectively) the elegance of the syntax of
Turing's model specification. It is close to how one would naturally write down
the model, and it is also the shortest. For those that are already familiar
with Julia, custom functions (such as the `stickbreak` function used) can be
implemented by the user and used in the model specification. Note that
functions from other libraries can be used quite effortlessly. For example, in
Turing, the `logsumexp` and `normlogpdf` methods are from the `StatsFuns.jl`
library. They worked without any tweaking in Turing. Another strength of Turing
sampling from the full conditionals of individual (or a group of) parameters 
using different inference algorithms is possible. For example, 
```julia 
...
Here are the inference times (in seconds) for each PPL for this model.  Smaller
is better. (By clicking the column headers, you can sort the rows by inference
times.)
{% include_relative ppl-snippets/dp-sb-gmm/timings/run-times.html %}

Here are the compile times (in seconds) for each PPL for this model.
(Again, smaller is better.) 
{% include_relative ppl-snippets/dp-sb-gmm/timings/compile-times.html %}

Note that when an inference algorithm was not implemented in a PPL, the timings
were recorded as "NA"; and when it *appeared* that an inference algorithm was
available in a PPL, but their usage was not well documented (i.e. documentation
was insufficient for me to implement/use), the timings were recorded as "DOC".
Both Numpyro and TFP are very promising PPLs, but still relatively young, so I
imagine syntax is still unstable and documentation is being updated constantly.
