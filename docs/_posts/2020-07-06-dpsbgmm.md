...
Here on, for brevity, $\mathbf{w} = \t{stickbreak}(\mathbf{v})$ will denote the
transformation lines 2-4 above.  Notice that the probability vector
$\mathbf{w}$ is $K$-dimensional, while $v$ is $(K-1)$-dimensional. (A simplex
$\mathbf{w}$ of length $K$ only requires $K-1$ elements to be specified. The
